...
public class Polygon {
  ...
  methods in this class that are not private. They can't be used anywhere else.
	...
	public void paintShip(Graphics brush){
		 Point shipShape[] =  getPoints(); 
		 
		 int[] xArray;
		 xArray = new int [4];//8 because there are 8 x and 8 y coordinates.
		 int[] yArray;
		 yArray = new int [4];
		 
		 for (int k = 0; k<shipShape.length;k++){
			xArray[k]  = (int) shipShape[k].getX();
			yArray[k]  = (int) shipShape[k].getY();
		 }

		 brush.drawPolygon(xArray,yArray,xArray.length);
		 brush.fillPolygon(xArray,yArray,xArray.length);
	}
	public void paintAsteroid(Graphics brush){
		 Point shipShape[] =  getPoints(); 
		 
		 int[] xArray;
		 xArray = new int [5];//8 because there are 8 x and 8 y coordinates.
		 int[] yArray;
		 yArray = new int [5];
